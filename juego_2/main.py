...
while running:
    ...
    if (
        ((player_1.get_right() >= ball.get_left()) and (player_1.get_left() <= ball.get_right()) ) and (
            (ball.get_bottom() >= player_1.get_top()) and (ball.get_bottom() <= player_1.get_bottom()) or (
                (ball.get_top() <= player_1.get_top ()) and (ball.get_top() <= player_1.get_bottom)
            )
        )
    ):
